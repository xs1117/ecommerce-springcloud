...
const PRE_PAY_STATUSES = ['CREATED', 'STOCK_CONFIRMED', 'WAIT_PAY'];
...
const isPayPreparing = computed(() => PRE_PAY_STATUSES.includes(order.value.status) && !canPay.value);
const canRequestRefund = computed(() => order.value.status === 'TO_SHIP' || order.value.status === 'TO_RECEIVE');
...
const syncOrderAutoRefresh = () => {
  const status = detail.value?.order?.status;
  if (PRE_PAY_STATUSES.includes(status) && !paying.value && !polling.value) {
...
const applyAfterSale = async () => {
  if (!order.value.orderNo || !canRequestRefund.value) {
    return;
  }
  loading.value = true;
  errorMessage.value = '';
  ...
  try {
    ...
    successMessage.value = order.value.status === 'TO_SHIP' ? '已提交退款申请' : '已提交售后申请';
    await loadDetail(true);
...
watch(() => route.params.orderNo, async (next, prev) => {
  if (!next || next === prev) {
    return;
  }
  clearOrderPolling();
    ...
    <main class="container detail-main">
      ...
      <template v-else>
        <section class="card info-panel" v-if="order.orderNo">
          ...
          <div class="pay-actions refund-actions" v-if="order.status === 'TO_SHIP'">
            <button class="btn btn-outline" :disabled="loading" @click="applyAfterSale">申请退款</button>
          </div>
          ...
          <div class="pay-actions" v-if="PRE_PAY_STATUSES.includes(order.status)">
            <button class="btn btn-success" :disabled="paying || closing || !canPay" @click="payNow">{{ paying ? '支付中...' : (canPay ? '立即支付' : '准备支付中...') }}</button>
            <button class="btn btn-outline" :disabled="paying || closing || polling || !canCancel" @click="cancelNow">{{ closing ? '关闭中...' : '取消支付' }}</button>
          </div>
          <p v-if="isPayPreparing" class="hint">订单正在确认中，支付按钮会在可支付后自动激活。</p>
...
.refund-actions { justify-content: flex-start; }
.refund-actions .btn { min-width: 120px; }
